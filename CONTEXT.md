...
**Estado:** Rama activa **`feature/getting-started-checklist`**, sincronizada con `origin`. Sesión 75 (commit `56ea29e`) fue **RECHAZADA por Codex Project Audit (P1)**: las clases sin horario (`schedule: null`/`[]`, p. ej. "Graduación"/"Open Mat") se listaban en el selector del CTA como filas con el mismo aspecto clicable que las reales, pero pulsarlas no producía ninguna respuesta — `buildBookingSession()` devolvía `null` y el `onClick` no hacía nada visible. Corregido en `e29e496`: `lib/trialBooking.ts` añade `hasBookableSchedule()` y `selectBookingSession()`; `TrialBookingCTA.tsx` ahora renderiza esas filas como `disabled`, sin chevron, con el texto "No online schedule — contact us" en vez de un botón activo silencioso. 8 tests nuevos cubren `schedule: null`, `schedule: []`, selección dentro de listas mixtas y el caso reservable. **Codex re-auditó `e29e496` y emitió `APROBADO CON PENDIENTES`** — confirmó independientemente que no hay hover engañoso/chevron/handler activo en filas no reservables, que las clases reservables siguen abriendo el modal, listas mixtas cubiertas, y todos los checks (`check-types`, Vitest 701/1, lint, `git diff --check`) correctos. Único hallazgo restante: P3 documental (pendientes explícitos por declarar, sin re-auditoría funcional) — resuelto en esta misma actualización. Lotes A, B y C quedan **`APROBADO CON PENDIENTES`**, sin hallazgos bloqueantes abiertos. La re-verificación de `/my` sigue completa a nivel de código/tests, pero no end-to-end autenticado. El trabajo ajeno `apps/web/app/api/claim/request/` y `apps/web/app/claim/page.tsx` sigue intacto. **La auditoría global NO está completa:** faltan pruebas con sesión autenticada real de `/my`/reserva/registro, la rama real `hasFreeTrialCls: true` con clases `isTrial`, y el ciclo de confirmación por email real — ver `docs/PROJECT-AUDIT.md` para el detalle de los tres pendientes explícitos y quién puede desbloquearlos. Pendientes previos: abrir PR de `feature/getting-started-checklist`, probar su flujo completo, sandbox Revolut no soportado, unificación `martial_active_context`/`currentSchoolId` sin decidir y confirmación visual mobile del sidebar.
...
**Resuelve el "Único pendiente" dejado abierto por la Sesión 76 de abajo:** esa sesión detectó que `463056e` había marcado 290 `SchoolMember` inactivos por una heurística de fecha poco fiable, y dejó dicho que la reconciliación real dependía de "la lista real de alumnos activos en V1 que el usuario va a exportar". Esta sesión es exactamente esa reconciliación, usando `users (9).csv` como esa lista.

**Parte 1 — login social OAuth atascado en bucle (rama `main`, commit `c9e9601`, mergeado/pusheado).**
- Causa raíz #1: `@supabase/ssr` fuerza `flowType: "pkce"` en `createBrowserClient` (no configurable) — el retorno de OAuth llega como `?code=...` en la query string, no `#access_token=...` en el hash. `LoginModal.tsx` y `app/login/page.tsx` solo comprobaban el hash (flujo implicit), así que la sesión se creaba correctamente pero la app nunca lo detectaba y nunca redirigía — el usuario se quedaba atascado en `/login`. Corregido: la señal de retorno OAuth ahora se captura de forma síncrona en el primer render mirando `?code=`/`?error=` en la query string, antes de que el SDK los borre de forma asíncrona.
- Causa raíz #2 (hallada por auditoría de Codex Project Audit sobre el primer fix, `RECHAZADO` P1/P2): `resolveDbUser()` en `lib/auth/server.ts` solo buscaba usuarios existentes — un alta nueva por Google/Apple/Microsoft creaba el `auth.users` de Supabase pero nunca el `User` de Prisma, así que `getAuthUser()` devolvía `null` para siempre. Corregido: `resolveDbUser()` aprovisiona el `User` (rol `STUDENT` por defecto) la primera vez que entra alguien por un proveedor OAuth (`app_metadata.provider` ∈ {google, apple, azure}), con manejo de condición de carrera (`P2002` → re-fetch y enlaza). También se añadió el `login-event` que faltaba en el flujo OAuth (antes solo lo mandaba el login por contraseña).
- Verificación: suite completa Vitest **739/739 passing** tras el primer fix, **756/756 passing, 1 todo** tras el segundo; `check-types` limpio en ambos.
- **Pendiente:** recorrido end-to-end con cuenta real de Google/Apple/Microsoft nueva y existente no realizado por Claude (no introduce credenciales reales en ningún formulario, por sus propias reglas de seguridad) — queda como criterio de la próxima verificación en navegador con sesión real.

**Parte 2 — corrección de `SchoolMember.status` para los alumnos de Roger Gracie Málaga importados de V1 (worktree `.claude/worktrees/rga-active-status`, rama `fix/rga-active-status`, **solo dry-run ejecutado, sin aplicar en vivo**).**
- Problema: `scripts/sync-rga-members.mjs` (y un import masivo anterior, sin tag) ponían `status: 'ACTIVE'` a **todos** los alumnos importados de V1 sin mirar su estado real — 436 de 726 `school_members` de RGA estaban ACTIVE en V2 cuando la interfaz real de V1 solo muestra 121 activos.
- Investigación del desfase 121 (V1 UI) vs 125 (`member_status=active` en `users (9).csv`): 125 − 1 (cuenta propia del negocio en V1, `user_type=2`, id V1 `798`, coincide con `School.v1UserId`) − 3 (cuentas hijas con `parent_id` set, que V1 no cuenta por separado en su panel) = 121. **Decisión confirmada con el usuario:** las 3 cuentas hijas sí quedan `ACTIVE` en V2 (V2 no tiene concepto de cuenta familiar y sí entrenan) → el objetivo real en V2 es **124**, no 121.
- Sin acceso al código fuente de V1 (Laravel) en esta máquina — evidencia triangulada por datos (CSV cruzado), no confirmada contra la query real; documentado como limitación.
- Cruce V2 → V1: preferentemente por `SchoolMember.notes = 'v1_student:<id>'` (solo 41 de 726 lo tienen — el import masivo original no usaba ese marcador); resto por email contra `users (9).csv`, excluyendo 2 emails ambiguos (comparten id) del mapa de respaldo. Los 2 `SchoolMember.role='OWNER'` quedan excluidos siempre, sin importar coincidencia de email.
- Revisado el impacto de `ARCHIVED` sobre otras entidades antes de tocar nada (`requireSchoolAccess()` y `hasStudentAccess()` sólo comprueban `SchoolMember.status`, no `Membership.status`; `assignPlan()` nunca reactiva un `ARCHIVED`; `findMembershipStatusDrift()` excluye `ARCHIVED` de su barrido) — **no hace falta armonizar ninguna otra entidad**, el cambio de `SchoolMember.status` es suficiente y ya está protegido contra reactivación accidental.
- Script nuevo `scripts/fix-rga-active-status.mjs` + lógica pura en `scripts/lib/rga-active-status.mjs` (testeada, 17 tests nuevos en `apps/web/__tests__/rgaActiveStatus.test.ts` — selección, alcance del matching, idempotencia). Idempotente (`--live` por defecto en modo dry-run; sin la flag no escribe nada), solo toca RGA, solo `role=STUDENT`, nunca borra ni toca `User`/`Membership`/`Transaction`/`Booking`.
- **Resultado del dry-run** (contra la base real): 720/726 emparejados a V1 (41 por notes, 679 por email); 2 excluidos por rol no-STUDENT; 4 sin match (no inequívocamente V1, no tocados); estado final tras aplicar → **122 ACTIVE, 598 ARCHIVED** (de los 124 definitivos: 1 sin ningún `SchoolMember` en V2 todavía — id V1 `997` — y 1 emparejado solo a la cuenta OWNER, no a un alumno — id V1 `844`, ambos documentados, ninguno creado ni tocado por este script).
- Verificación: 17/17 tests nuevos ✅, suite completa **756/756 passing, 1 todo** ✅, `check-types` ✅, `git diff --check` ✅. `scripts/` no tiene lint propio en este repo (sin `eslint.config` en la raíz, no forma parte de ningún paquete de Turborepo — mismo patrón que el resto de `scripts/`).
- Hay otra sesión en paralelo trabajando en `.claude/worktrees/rga-belts-scope` (rama `worktree-rga-belts-scope`) sobre `sync-rga-members.mjs`/`sync-rga-payments.mjs` (scope de cinturones, no relacionado) — sin colisión de archivos con este cambio.

**Modo `--live` ejecutado con aprobación explícita del usuario, tras investigar los dos casos pendientes:**
- **V1 id 844** (`p.cabomedina@gmail.com`, dueño de la sesión): resuelto sin acción — en V1 era alumno activo, pero en V2 ya existe únicamente como cuenta `OWNER` de la escuela (sin `SchoolMember` de tipo `STUDENT`). Excluido correctamente por el guard de rol.
- **V1 id 997** (`pablo_ildue@hotmail.com`): email duplicado en el export de V1 entre dos identidades distintas — id 991 (`user_type=2`, `member_status=lead`, `deleted_at` set, staff/soft-deleted) e id 997 (`user_type=3`, `member_status=active`, el alumno real). El único `User` de V2 con ese email tiene nombre "Pablo Vilachá Calviño" (coincide con el 991 borrado) pero un `SchoolMember` `STUDENT`/`ACTIVE` sin `notes` — ambigüedad real irresoluble con los datos disponibles sobre a cuál de los dos representa. **Decisión del usuario: dejarlo tal cual (ACTIVE, sin tocar)** — comportamiento que el script ya aplicaba por defecto al no poder emparejarlo de forma inequívoca.
- Corregido el rótulo de log `"Final state after this run"` → `"Final state among matched V1 students"` (el número solo sumaba la población emparejada — 122 —, no los 726 completos; no era un bug de escritura, solo una etiqueta ambigua en el resumen impreso).
- **Verificación post-ejecución contra la base real:** `school_members` de RGA: **726 totales, 128 ACTIVE, 598 ARCHIVED, 0 en cualquier otro estado**. De los 128 ACTIVE: 122 alumnos verificados contra la lista definitiva de V1, 2 cuentas `OWNER` (nunca tocadas) y 4 cuentas sin emparejar que ya estaban ACTIVE y se dejaron intactas a propósito (incluye el caso ambiguo de arriba y un alta self-service "Join this school" genuina de V2). 122+2+4=128, 128+598=726 — cuadra exactamente.
- **Segunda ejecución (`--live` de nuevo): `Updates queued: 0`** — idempotencia confirmada contra la base real, no solo en tests.
- Commits en `fix/rga-active-status`: `6bbc53a` (script + tests, dry-run), `3a8c831` (resultado del live + fix de rótulo de log). PR [#19](https://github.com/MartialOneOnline/martial-v2/pull/19) creado y auditado por Codex Project Audit.
- **Veredicto Codex: `APROBADO CON PENDIENTES`.** Confirmó de forma independiente: aritmética 125→121→124 reproducible, script solo lee/escribe dentro de los 726 `school_members` de RGA, solo `role=STUDENT`, única escritura posible es `school_members.update({status})`, sin crear/borrar filas ni tocar `User`/`Membership`/`Transaction`/`Booking`, prioriza `notes` sobre email, fallback de email descarta los 2 duplicados del CSV, casos 844/997 tratados según lo documentado, dry-run independiente contra producción coincide exactamente (720 emparejados, 122 ACTIVE/598 ARCHIVED, 0 pendientes), type-check/Vitest 756+1todo/`git diff --check` en verde, rama sincronizada. Único hallazgo: **P3** — el log de dry-run decía "no matching V2 account at all" para el id V1 997 cuando en realidad sí existe una cuenta V2 con ese email (solo que no se puede atribuir sin ambigüedad, por el email duplicado en V1) — corregido en `98c8186` a "no unambiguous V2 match", sin cambio de lógica ni de resultado. Codex indicó que no requiere reauditoría funcional.
- Limitación aceptada por Codex: la exclusión de cuentas hijas en el panel de V1 está respaldada por los datos, no por la consulta Laravel original (no disponible).
...
**Contexto de `463056e`:** alumnos de Roger Gracie Málaga reportaron pago desactualizado y graduación incorrecta tras el import V1→V2. Causa raíz real: el parser CSV casero de `scripts/sync-rga-*.mjs` no manejaba el escapado RFC4180 de comillas dobles (`userdetails.belts` viene como `"{""18"":""332""}"`), así que la columna JSON con el cinturón real salía corrupta y todos los alumnos importados caían en el default Blanco/0. `463056e` corrigió el parser y añadió la resolución de cinturón vía `belt_ranks.id`, pero **además** modificó `sync-rga-payments.mjs` para derivar `Membership.status = EXPIRED` a partir de `expires_at` de la última reserva V1, y aplicó ambos cambios en vivo (291 memberships ACTIVE→EXPIRED, 290 `SchoolMember` ACTIVE→INACTIVE). El usuario, tras comparar el conteo activo/inactivo real de V1 (`legacy.martialapp.com`: 121 activos / 582 inactivos de 703) contra el resultado en V2 (434 activos de 724), confirmó que esa heurística por fecha no reproduce fielmente el estado real de V1 — se acordó que el status de membership se resolverá más adelante con una lista de activos exportada directamente de V1, y que mientras tanto el cron existente (con su período de gracia de 30 días) es la única fuente de verdad para expirar membresías. Este lote **no toca ese dato** — solo reduce el commit ya en `main` a su parte correcta (cinturones).

- `scripts/sync-rga-payments.mjs`: revertido a su versión previa a `463056e` — `mapMembershipStatus(v1Status)` vuelve a depender solo del código de status V1 (CANCELLED si `3`/`4`, si no ACTIVE), sin `expires_at`. Diff contra `463056e^` para este archivo: vacío (byte-idéntico).
- `scripts/lib/rga-belts.mjs` (nuevo): extrae como helpers puros y testeables `parseLine`/`parseCSV` (parser RFC4180-aware), `mapBelt`, `parseBeltRankTitle` (color + grado desde el título de `belt_ranks`, incluye el fix del typo V1 `Nagro`→`Negro`), `resolveBeltRankId` (JSON `{"18": id}` con id string o numérico, o array) y `resolveBelt` (pipeline completo). Usado por `sync-rga-members.mjs` y por el nuevo `fix-rga-belts.mjs`.
- `scripts/sync-rga-members.mjs`: sin cambios de comportamiento — ahora importa los helpers desde `lib/rga-belts.mjs` en vez de definirlos inline. Conserva el parser con comillas duplicadas, la lectura de `userdetails.belts`, la resolución vía `belt_ranks.id` y el mapeo de color/grado.
- `scripts/fix-rga-belts-and-memberships.mjs` **eliminado**, reemplazado por `scripts/fix-rga-belts.mjs`: mismo propósito de reparación de datos ya escritos, pero con alcance exclusivo a `belt`/`beltDegree`/`updatedAt` sobre `school_members` de RGA etiquetados `notes = 'v1_student:*'`. Eliminadas por completo las consultas/actualizaciones de `memberships`, los cambios de `SchoolMember.status` y la función `mapMembershipStatus` que tenía el script anterior.
- `scripts/__tests__/rga-belts.test.mjs` (nuevo, `node:test` nativo — `scripts/` no es parte del workspace de Vitest de `apps/web`): 22 tests cubriendo el parser con comillas duplicadas (`"{""18"":""332""}"`), IDs JSON numérico y string, Blanco sin grado, Azul/Morado/Marrón/Negro con grado, el typo `Nagro 4 Grados`, `belts` vacío/`NULL`/`{}`/`[]`/JSON inválido, y rank id inexistente.
- Verificado: `node --check` en los 5 archivos `.mjs` tocados ✅; `node --test scripts/__tests__/rga-belts.test.mjs` 22/22 ✅; `npm --workspace web run check-types` ✅; `npm --workspace web test -- --run` 739 passing/1 todo (76 archivos) ✅; `npm --workspace web run lint` 0 errores/235 warnings (baseline preexistente, ninguno en archivos tocados) ✅; `git diff --check` limpio ✅.
...
- Navegador: `/login` en desktop y móvil (pantalla directa al email, sin SSO) ✅; Forgot Password → formulario real → Back sin romper estado ✅ (no se completó el envío real del email para no disparar una operación externa); página pública de RGM en viewport móvil (375×812) ✅; CTA sticky móvil "Reservar clase" → selector con las 6 clases reales, incluidas "Graduación"/"Open Mat" sin horario, **mostradas sin crash — pero sin probar si eran pulsables** ✅; "Jiu Jitsu Avanzado" (Mon 19:00–20:30) → modal correcto pidiendo login/registro ✅. Sin errores en consola ni en logs del servidor.
- Sin tocar los untracked ajenos `apps/web/app/api/claim/request/` y `apps/web/app/claim/page.tsx` (confirmado antes y después).
- **Commit `56ea29e` entregado con este estado.**

**RECHAZADO por Codex Project Audit (P1) — commit `56ea29e`:** el punto anterior probó que "Graduación"/"Open Mat" se mostraban sin crashear, pero nunca probó pulsarlas. El código lo demostraba: `selectClass()` llamaba a `buildBookingSession()`, que devuelve `null` para clases sin horario, y el `if (s) {...}` hacía que el `onClick` no produjera ninguna respuesta — mismo botón, mismo hover, mismo chevron que una fila reservable, pero un clic muerto. Exactamente el patrón de affordance falso que esta misma serie de sesiones llevaba encontrando en otras superficies.

**Corrección entregada en el mismo lote:**
- `lib/trialBooking.ts`: nuevas `hasBookableSchedule()` (true solo si `schedule.length > 0`) y `selectBookingSession(classes, classId)` (resuelve por id dentro de una lista, incluyendo las no reservables).
- `TrialBookingCTA.tsx`: las filas sin horario ahora se renderizan `disabled`, sin `ChevronRight`, con fondo gris y el texto "No online schedule — contact us"; `handleClick()` ya no abre el modal en falso cuando la única clase de la lista no es reservable (cae al chooser, que la muestra deshabilitada, en vez de que el botón principal tampoco responda).
- 8 tests nuevos: `hasBookableSchedule` para `null`/`[]`/slot real; `buildBookingSession` con `schedule: []` (antes solo se cubría `null`); `selectBookingSession` con una lista mixta reservable/no-reservable, incluyendo selección explícita de la clase no reservable.
- Re-verificado: `check-types` ✅, Vitest **701 passing, 1 todo** ✅, `eslint` ✅, `git diff --check` ✅, y navegador móvil pulsando específicamente "Graduación" y "Open Mat" (sin respuesta, correcto) y una clase reservable distinta a la ya probada antes ("Jiu Jitsu Iniciación" → modal correcto). Sin errores de consola.
- Corregidas las dos afirmaciones que se contradecían con el hallazgo de Codex (esta misma entrada y `docs/PROJECT-AUDIT.md`, Lote C) — decían "sin crash" dando a entender que el recorrido estaba probado end-to-end, cuando solo se había probado la ausencia de excepción, no el resultado del clic.

**Veredicto final — Codex Project Audit sobre `e29e496`: `APROBADO CON PENDIENTES`.** Confirmó de forma independiente cada punto del delta correctivo (sin hover engañoso/chevron/handler activo en filas no reservables, clases reservables siguen funcionando, listas mixtas cubiertas, checks en verde, `claim/` intacto). Único hallazgo restante fue P3 documental — pendientes explícitos sin declarar con claridad, resuelto en esta misma sesión sin necesitar otra auditoría funcional. Lotes A, B y C cierran en `APROBADO CON PENDIENTES`; los tres pendientes que quedan fuera de alcance (escuela con `hasFreeTrialCls: true`, reserva con sesión autenticada real, ciclo de registro con correo real) están documentados en `docs/PROJECT-AUDIT.md` como el criterio para priorizar el siguiente lote.
