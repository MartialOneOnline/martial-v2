...
export async function POST(req: NextRequest) {
  ...
  // Sniff the real file signature rather than trusting the client-declared
  // File.type — that's just a string the caller sets, spoofable by renaming
  // a script or crafting a raw request. See validateImageUpload.ts.
  ...
  // Path is fully server-derived (userId + timestamp + the sniffed
  ...
  // File.name.
  const path = `${user.id}-${Date.now()}.${sniffed.ext}`
