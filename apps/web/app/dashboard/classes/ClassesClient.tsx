...
import {
  ...
  ChevronLeft, TrendingUp, Check, Upload,
  Clock,
} from 'lucide-react'
...
function NavGroup({ item }: { item: NavItem }) {
  const isParentActive = item.children?.some(c => c.href === '/dashboard/classes')
  ...
  return (
    <div>
      <button onClick={() => setOpen(v => !v)}
        ...
        style={{ color: '#374151', fontSize: 14, background: isParentActive ? '#EFF6FF' : 'transparent', border: 'none' }}
        onMouseEnter={e => { if (!isParentActive) (e.currentTarget as HTMLElement).style.background = '#F9FAFB' }}
        onMouseLeave={e => { if (!isParentActive) (e.currentTarget as HTMLElement).style.background = isParentActive ? '#EFF6FF' : 'transparent' }}
      >
        <item.icon size={16} style={{ color: isParentActive ? '#0071E3' : '#9CA3AF', flexShrink: 0 }} />
        ...
        <div className="ml-7 mt-0.5 space-y-0.5">
          {item.children!.map(child => (
            <Link key={child.label} href={child.href}
              ...
              style={{ fontSize: 13, color: child.href === '/dashboard/classes' ? '#0071E3' : '#6B7280',
                fontWeight: child.href === '/dashboard/classes' ? 600 : 400 }}
              onMouseEnter={e => { if (child.href !== '/dashboard/classes') { (e.currentTarget as HTMLElement).style.background = '#F9FAFB'; (e.currentTarget as HTMLElement).style.color = '#111827' }}}
              onMouseLeave={e => { if (child.href !== '/dashboard/classes') { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = '#6B7280' }}}
            >{child.label}</Link>
          ))}
        </div>
      )}
