    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4">
          <h2 className="text-xl font-bold text-[#101828]">School Invitations</h2>
          ...
          {([
            ...
            <button
              ...
              className={`pb-3 px-1 mr-6 text-sm font-semibold border-b-2 transition-colors ${
                tab === t.key ? 'border-[#0870E2] text-[#0870E2]' : 'border-transparent text-gray-400 hover:text-gray-600'
              }`}
            >
              {t.label}
            </button>
        ...
        <div className="p-6">
          ...
          {tab === 'invite' && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-[#101828] mb-1.5">Business Name</label>
                <input
                  value={inviteName}
                  onChange={e => setInviteName(e.target.value)}
                  placeholder="Enter the invitee name"
                  className="w-full h-12 px-4 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2] focus:ring-2 focus:ring-[#0870E2]/10"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-[#101828] mb-1.5">Email Address</label>
                <input
                  type="email"
                  value={inviteEmail}
          ...
          {tab === 'import' && (
            <div className="space-y-4">
              ...
              <div
                ...
                className="relative border-2 border-dashed border-[#0870E2]/30 bg-[#F0F7FB] rounded-xl p-8 flex flex-col items-center gap-3 cursor-pointer hover:border-[#0870E2]/60 hover:bg-[#E8F3FA] transition-colors"
              >
                <input ref={fileRef} type="file" accept=".xlsx,.csv,.xls" className="hidden" onChange={handleFileInput} />
                <div className="text-4xl">📁</div>
                <div>
                  <p className="text-sm font-bold text-[#101828] text-center">
                ...
                {!importFile && (
                  <>
                    ...
                    <div className="flex gap-3 text-xs font-semibold" style={{ color: '#0870E2' }}>
                      <span>.XLSX</span><span>.CSV</span><span>Max 5MB</span>
                    </div>
                  </>
                )}
                {importRows.length > 0 && (
                  <div className="w-full mt-1 max-h-28 overflow-y-auto rounded-lg border border-[#0870E2]/20 bg-white">
                    {importRows.slice(0, 5).map((r, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-1.5 border-b border-gray-50 last:border-0 text-xs">
                        <span className="font-medium text-[#101828] truncate">{r.name}</span>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  style={{ background: importResult ? '#10B981' : '#0870E2' }}
                >
            ...
            <div className="space-y-5 max-h-[70vh] overflow-y-auto pr-1">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <div
                      ...
                      className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center cursor-pointer hover:border-[#0870E2]/50 hover:bg-[#F0F7FB] transition-colors overflow-hidden"
                    >
                      {logoPreview
                  ...
                  <div className="flex-1 flex flex-col gap-1.5">
                    <div
                      ...
                      className="h-20 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center cursor-pointer hover:border-[#0870E2]/50 hover:bg-[#F0F7FB] transition-colors overflow-hidden"
                    >
                      {coverPreview
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <input value={addForm.name} onChange={e => setAddForm(f => ({...f, name: e.target.value}))}
                      placeholder="Business Name *" className="w-full h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  </div>
                  <input value={addForm.email} onChange={e => setAddForm(f => ({...f, email: e.target.value}))}
                    placeholder="Email" type="email" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <input value={addForm.phone} onChange={e => setAddForm(f => ({...f, phone: e.target.value}))}
                    placeholder="Phone Number" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <input value={addForm.tagline} onChange={e => setAddForm(f => ({...f, tagline: e.target.value}))}
                    placeholder="Tagline" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <input value={addForm.foundedYear} onChange={e => setAddForm(f => ({...f, foundedYear: e.target.value}))}
                    placeholder="Founded Year" type="number" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input value={addForm.address} onChange={e => setAddForm(f => ({...f, address: e.target.value}))}
                    placeholder="Address" className="col-span-2 h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <input value={addForm.city} onChange={e => setAddForm(f => ({...f, city: e.target.value}))}
                    placeholder="City" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <input value={addForm.postcode} onChange={e => setAddForm(f => ({...f, postcode: e.target.value}))}
                    placeholder="Post Code" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  <select value={addForm.country} onChange={e => setAddForm(f => ({...f, country: e.target.value}))}
                    className="col-span-2 h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2] bg-white">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="relative">
                    ...
                    <input value={addForm.instagram} onChange={e => setAddForm(f => ({...f, instagram: e.target.value}))}
                      placeholder="Instagram" className="w-full h-10 pl-8 pr-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  </div>
                  <div className="relative">
                    ...
                    <input value={addForm.facebook} onChange={e => setAddForm(f => ({...f, facebook: e.target.value}))}
                      placeholder="Facebook" className="w-full h-10 pl-8 pr-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  </div>
                  <div className="relative">
                    <Link className="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
                    <input value={addForm.youtube} onChange={e => setAddForm(f => ({...f, youtube: e.target.value}))}
                      placeholder="YouTube" className="w-full h-10 pl-8 pr-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  </div>
                  <div className="relative">
                    <Globe className="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" />
                    <input value={addForm.website} onChange={e => setAddForm(f => ({...f, website: e.target.value}))}
                      placeholder="Website" className="w-full h-10 pl-8 pr-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
                  </div>
                </div>
              </div>
              ...
              <div>
                <textarea value={addForm.description} onChange={e => setAddForm(f => ({...f, description: e.target.value}))}
                  ...
                  rows={3}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2] resize-none" />
              </div>

              {/* Status + Pricing */}
              <div className="grid grid-cols-3 gap-3">
                <select value={addForm.status} onChange={e => setAddForm(f => ({...f, status: e.target.value}))}
                  className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2] bg-white">
                  <option value="UNVERIFIED">Unverified</option>
                  <option value="CLAIMED">Claimed</option>
                  <option value="VERIFIED">Verified</option>
                </select>
                <input value={addForm.priceFrom} onChange={e => setAddForm(f => ({...f, priceFrom: e.target.value}))}
                  placeholder="Price from (£)" type="number" className="h-10 px-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#0870E2]" />
              ...
              <div className="flex gap-3 pt-1 pb-1">
                ...
                <button
                  ...
                  className="flex-1 h-11 rounded-xl text-white text-sm font-bold uppercase tracking-wide flex items-center justify-center gap-2 hover:opacity-90 transition-opacity disabled:opacity-50"
                  style={{ background: addSuccess ? '#10B981' : '#0870E2' }}
                >
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:border-[#0870E2]"
          />
        </div>
        <div className="relative">
          <select
            ...
            className="h-10 pl-3 pr-8 rounded-xl border border-gray-200 bg-white text-sm font-medium appearance-none focus:outline-none focus:border-[#0870E2] cursor-pointer"
          >
              ...
              return (
                <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3.5">
                    <p className="font-semibold text-[#101828]">{inv.name}</p>
