    ...
    <section className="bg-[#EEF5FB] py-20 sm:py-28" id="technology">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="text-center mb-14">
          <h2 className="text-3xl sm:text-4xl font-bold text-[#0D1B2A] leading-snug">
            A FREE to user Platform for
          </h2>
          <p className="text-3xl sm:text-4xl font-bold text-[#006197] mt-1">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="flex flex-col items-start gap-8">
            <p className="text-base font-semibold text-gray-500 border-l-2 border-[#006197] pl-4 max-w-xs leading-snug">
                ...
                <div className="rounded-[1.8rem] overflow-hidden bg-white border border-gray-100">
                  ...
                  <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-md bg-[#006197] flex items-center justify-center">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-[#006197]/10 flex items-center justify-center overflow-hidden">
                        <span className="text-[9px] font-bold text-[#006197]">AQ</span>
                      </div>
                    ...
                    <div className="flex items-center bg-white p-0.5 rounded-lg border border-gray-200 gap-0.5">
                      {(['students', 'payments', 'schedule'] as const).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                          className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition-all cursor-pointer capitalize ${activeTab === tab ? 'bg-[#006197] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
                      ...
                      {activeTab === 'students' && (
                        <motion.div key="students" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-2">
                          <div className="flex items-center justify-between pb-1.5 border-b border-gray-100">
                            ...
                            <span className="bg-[#006197]/10 text-[#006197] text-[8px] font-bold px-2 py-0.5 rounded-md">4 Active Now</span>
                          </div>
                          {STUDENTS.map((s, i) => (
                            <div key={i} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg border border-gray-100">
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-[#006197]/10 flex items-center justify-center text-[9px] font-bold text-[#006197]">{s.name[0]}</div>
                        ...
                        <motion.div key="schedule" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-2">
                          ...
                          {SCHEDULE.map((s, i) => (
                            <div key={i} className="p-2 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-between">
                              ...
                              <div className="text-right"><span className="text-[8px] font-bold bg-[#006197]/10 text-[#006197] px-1.5 py-0.5 rounded-md">{s.time}</span><p className="text-[8px] text-gray-400 mt-1">{s.attendees} attendees</p></div>
                            </div>
                          ))}
                        </motion.div>
                      )}
            ...
            <div className="space-y-5">
              {TICKS.map((tick, i) => (
                <div key={i} className="flex gap-3 items-start group">
                  <CheckCircle2 className="w-5 h-5 text-[#006197] shrink-0 mt-0.5 group-hover:scale-110 transition-transform" />
        ...
        <div className="text-center mt-14">
          <a href="#register"
            className="inline-flex items-center justify-center px-10 py-4 bg-[#006197] text-white font-extrabold text-[15px] rounded-xl hover:bg-[#004d79] shadow-md transition-all cursor-pointer active:scale-95">
